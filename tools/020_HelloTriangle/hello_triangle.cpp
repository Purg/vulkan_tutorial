 ...
 *
 * NOTES:
 *   - Given the vulkan documentation for `VkApplicationInfo`, does that mean that `pEngineName`
 *     should be "constant" if generated by "myengine"? Does this not matter if we're defining it
 *     here? Seems like this would be defined by the library providing the wrapping around vulkan
 *     while the user utilizing that library provides the application name/version.
 *
