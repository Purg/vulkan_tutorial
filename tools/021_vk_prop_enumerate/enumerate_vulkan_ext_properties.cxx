...
main()
{
  ...
  {
    LOG_ERROR(
      "Failed to create Vulkan instance for physical device query: " << res );
    return 1;
  }
  ...
  for( auto const& device : pdevice_vec )
  {
    ...
    LOG_INFO( "     Properties:" );
    LOG_INFO( "       API version supported: "  <<
              VK_VERSION_MAJOR( pd_prop.apiVersion )
                                                << "." <<
    ...
    auto queue_props_vec( myengine::vulkan::get_device_queue_family_properties(
                            device ) );
    ...
    for( auto const& queue_props : queue_props_vec )
    {
      LOG_INFO( "      "        << queue_props.queueCount << "x \tw/ Flags: "
                                << std::showbase <<
                std::bitset< 8 >( queue_props.queueFlags ) );
    }
  }
